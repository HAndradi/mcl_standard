...
void cloudCb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg, ParticleFilter &pf_standard){
    ...
    transform.setOrigin(tf::Vector3(avg_particle_pos(0), avg_particle_pos(1), 0.0));
    tf::Quaternion tf_quat;
    quaternionMsgToTF(pose_msg.pose.pose.orientation, tf_quat);
    transform.setRotation(tf_quat);
    tf_broadcaster_ptr->sendTransform(tf::StampedTransform(transform, cloud_msg->header.stamp, "map", "rslidar"));
}
